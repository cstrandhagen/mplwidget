...
class NavigationToolbar(QtGui.QToolBar):
    ...
    def __init__(self,canvas,parent,coordinates=False):
        ...
        self.backAction = self.addAction(get_icon('arrow_left'),'Back',self.back)
        self.backAction.setToolTip('restore last view')
        self.forwardAction = self.addAction(get_icon('arrow_right'),'Forward',self.forward)
        self.forwardAction.setToolTip('go to next view')
        
        #register default mouse behaviour
        self._idPress = self.canvas.mpl_connect('button_press_event',self._on_click)
        self._idScroll = self.canvas.mpl_connect('scroll_event',self._on_scroll)
    ...
    def _on_scroll(self, event):
        ...
        self.scroll_zoom(event.inaxes, event.step, location=(event.xdata, event.ydata))
    ...
    def _middle_click(self,event):
        ...
        if event.inaxes is not None:
            ...
            self._idDrag = self.canvas.mpl_connect('mouse_move_event',self.drag_pan)
            x, y = event.x, event.y
    ...
    def _left_click(self, event):
        ...
        if ax.get_xaxis().contains(event)[0]:
            ...
            self.axzoom = AxisSpan(ax,event,zoom,'horizontal',minspan=0.001,color='w')
        if ax.get_yaxis().contains(event)[0]:
            ...
            self.axzoom = AxisSpan(ax,event,zoom,'vertical',minspan=0.001,color='w')
    ...
    def _right_click(self, event):        
        ...
        for artist in [self.canvas.figure.gca().xaxis.label, self.canvas.figure.gca().yaxis.label]:
            picked, props = artist.contains(event)
            if picked:
                sub_menus.append(SubMenu(artist,parent=self))
        ...
        menu.exec_(self.canvas.mapToGlobal(QtCore.QPoint(event.x, self.canvas.height() - event.y)))
...
class SubMenu(QtGui.QMenu):
    ...
    def createMenu(self):
        if hasattr(self.artist,'__name__'):
            if self.artist.__name__ in ['xaxis','yaxis']:
                a = self.addAction('edit axis label',
                                   partial(self.edit_text,self.artist.get_label_text,self.artist.set_label_text))
        ...
        if hasattr(self.artist,'set_text'):
            a = self.addAction('edit text', 
                               partial(self.edit_text, self.artist.get_text, self.artist.set_text))
    ...
    def _title(self):
        if hasattr(self.artist,'__name__'): return self.artist.__name__
        elif hasattr(self.artist,'get_label'): return self.artist.get_label()
        else: return str(self.artist)
    ...
    def pop_up(self):
        self.exec_(self.parent.canvas.mapToGlobal(QtCore.QPoint(self.event.mouseevent.x, self.parent.height() - self.event.mouseevent.y)))
...
def _convert_font_fromQT(font):
    ...
    weight_dict = {25:'light',50:'normal',63:'demibold',75:'bold',87:'black'}
...
def _convert_font_toQT(font):
    ...
    weight_dict = {'light':25,'normal':50,'demibold':63,'bold':75,'black':87}
